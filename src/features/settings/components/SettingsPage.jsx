...
export default function SettingsPage() {
  const { settings, loading, saving, updateProfile, updateAccount, updatePrivacy, updateNotifications, updateAppearance, changePassword, deleteAccount } = useSettings();
  ...
  const handleSave = async (section) => {
    try {
      ...
      setSaveMessage({ type: "error", message: "Error al guardar cambios" });
      setTimeout(() => setSaveMessage(null), 3000);
    }
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#0d0e14" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "40px" }}>
        ...
        {activeSection === "account" && <AccountSection settings={settings} onSave={handleSave} />}
        {activeSection === "privacy" && <PrivacySection settings={settings} onSave={handleSave} />}
        {activeSection === "notifications" && <NotificationsSection settings={settings} onSave={handleSave} />}
        {activeSection === "appearance" && <AppearanceSection settings={settings} onSave={handleSave} />}
...
function AccountSection({ settings, onSave }) {
  return (
    <div>
...
function PrivacySection({ settings, onSave }) {
  return (
    <div>
